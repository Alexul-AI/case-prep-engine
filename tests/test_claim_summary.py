...
class SupportedCaseTests(unittest.TestCase):
    ...
    def test_clean_summary_with_real_citation_passes(self):
        ...
        self.assertEqual(validate_claim_summary(summary, self.entry), [])
    ...
    def test_fake_citation_hash_is_rejected(self):
        ...
        problems = validate_claim_summary(summary, self.entry)
...
class CausalWordingTests(unittest.TestCase):
    ...
    def test_causal_wording_backed_by_cited_causal_payload_passes(self):
        entry = entry_for([self.causal_row])
        summary = ClaimSummary(
            claim_id="C99", status="supported",
        ...
        self.assertEqual(validate_claim_summary(summary, entry), [])

    def test_causal_wording_not_backed_by_any_cited_payload_is_rejected(self):
        entry = entry_for([self.noncausal_row])
        summary = ClaimSummary(
            claim_id="C99", status="supported",
        ...
        problems = validate_claim_summary(summary, entry)
        self.assertTrue(any("causal wording" in p for p in problems))

    def test_causal_payload_must_be_cited_not_merely_present_in_entry(self):
        # The causal-supporting document exists in the entry but isn't in
        # citations -- must still fail. Citing an uncited source doesn't count.
        entry = entry_for([self.causal_row, self.noncausal_row])
        summary = ClaimSummary(
            claim_id="C99", status="supported",
            summary_he="קיים קשר סיבתי בין האירוע למחלה",
            summary_ru="", allowed_uses=(), must_not_say=(),
...
class ContradictionAndConflictTests(unittest.TestCase):
    def test_contradiction_must_be_reflected_in_status_or_open_risks(self):
        ...
        entry = entry_for([support, contradiction])
        ...
        self.assertTrue(
            any("contradiction" in p for p in validate_claim_summary(silent, entry))
        )
        ...
        self.assertEqual(validate_claim_summary(as_status, entry), [])
        ...
        self.assertEqual(validate_claim_summary(as_open_risk, entry), [])
    ...
    def test_negative_finding_must_be_cited_not_just_acknowledged(self):
        ...
        self.assertTrue(
            any("negative finding" in p for p in validate_claim_summary(uncited, entry))
        )
        ...
        self.assertEqual(validate_claim_summary(cited, entry), [])
    ...
    def test_unresolved_conflict_forbids_supported_status(self):
        ...
        self.assertTrue(
            any("unresolved conflict" in p for p in validate_claim_summary(supported, entry))
        )
        ...
        self.assertEqual(validate_claim_summary(blocked, entry), [])
...
class BuildClaimSummaryPromptTests(unittest.TestCase):
    def test_prompt_contains_claim_id_and_hash_and_rules(self):
        ...
        entry = entry_for([row], claim_id="C08")
    ...
    def test_prompt_reflects_unresolved_conflict_flag(self):
        ...
        entry = entry_for([a, b], claim_id="C08")
        request = build_claim_summary_request(entry)
        prompt = build_claim_summary_prompt(request)
...
class RealRegisterClaimSummaryTests(unittest.TestCase):
    def test_real_gour_quote_validates_against_real_entry(self):
        rows = import_csv(REAL_REGISTER_CSV)
        matrix = build_evidence_matrix(rows)
        entry = matrix["C08"]
        gour_hash = entry.supporting[0].row.payload.payload_hash
